...
def check_rating_lock(user_id, movie_id):
    ...
    If locked <30s ago by another username, return that username.
    Else return None.
    """
    sql = """
...
def add_or_update_rating(user_id, movie_id, rating_val):
    """
    Upsert style rating write, but ACID:
    - BEGIN
    - DELETE old row
    ...
    Also prints [TRANSACTION]/[ROLLBACK] markers for report.
...
def get_user_statistics(user_id):
    ...
    sql = """
        SELECT
            ...
            (SELECT COUNT(*) FROM ratings WHERE rating >= 4.0 AND userId = u.userId)
                AS high_ratings_count,
            (SELECT COUNT(*) FROM ratings WHERE rating <= 2.0 AND userId = u.userId)
...
def get_popular_movies_from_view(limit=20):
    ...
    This demonstrates VIEW usage as requested by professor.
...
class MovieApp(tk.Tk):
    ...
    def build_users_tab(self):
        ...
        ttk.Label(rating_form, text="User ID/Name/Email:")\
            .grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.rate_user_id_var = tk.StringVar()
        self.rate_user_id_entry = ttk.Entry(
            rating_form, textvariable=self.rate_user_id_var, width=20
        )
        self.rate_user_id_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(rating_form, text="Movie ID or Title:")\
            .grid(row=0, column=2, sticky="e", padx=5, pady=5)
        self.rate_movie_id_var = tk.StringVar()
        self.rate_movie_id_entry = ttk.Entry(
            rating_form, textvariable=self.rate_movie_id_var, width=25
        )
        self.rate_movie_id_entry.grid(row=0, column=3, padx=5, pady=5)

        ttk.Label(rating_form, text="Rating (0.5-5.0):")\
            .grid(row=0, column=4, sticky="e", padx=5, pady=5)
        self.rate_value_var = tk.StringVar()
        self.rate_value_entry = ttk.Entry(
            rating_form, textvariable=self.rate_value_var, width=10
        )
        self.rate_value_entry.grid(row=0, column=5, padx=5, pady=5)

        # Rating buttons - separate row for better alignment
        rating_btn_frame = tk.Frame(rating_form, bg="white")
        rating_btn_frame.grid(row=1, column=0, columnspan=6, pady=15)

        self.btn_add_update_rating = ttk.Button(
            rating_btn_frame,
            text="Add/Update Rating",
            width=20,
            command=self.handle_add_update_rating
        )
        self.btn_add_update_rating.grid(row=0, column=0, padx=10, pady=0)

        self.btn_delete_rating = ttk.Button(
            rating_btn_frame,
            text="Delete Rating",
            width=20,
            command=self.handle_delete_rating
        )
        self.btn_delete_rating.grid(row=0, column=1, padx=10, pady=0)
        ...
        try:
            ...
            if ok:
                self._append_rating_log(
                    f"Upsert OK: user={uid}, movie={mid}, rating={rating_val}"
                )
            else:
                self._append_rating_log(
                    f"Upsert FAILED for user={uid}, movie={mid}"
                )
        finally:
            # always release
...
if __name__ == "__main__":
    # Uncomment to run transaction tests before GUI
